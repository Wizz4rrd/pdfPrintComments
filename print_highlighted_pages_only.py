#!/usr/bin/env python3
"""
Generate a printable PDF containing only pages that have highlight annotations,
with any associated comments rendered in a sidebar.

A page is included if it has at least one Highlight annotation, regardless of
whether that highlight carries a text comment.  Highlights with comments get
the usual sidebar treatment.

Usage:
    python print_highlighted_pages_only.py [input.pdf]
...
def has_highlights(page):
    """Return True if the page has at least one Highlight annotation."""
...
def process_pdf(input_path, output_dir):
    ...
    output_path = os.path.join(output_dir, f"{basename}_highlighted_pages.pdf")
    ...
    for i in range(total_pages):
        if has_highlights(src_doc[i]):
            highlighted_indices.append(i)

    if not highlighted_indices:
        print(f"  No pages with highlights — skipping.")
        src_doc.close()
        return

    dst_doc = fitz.open()
    for src_idx in highlighted_indices:
        dst_doc.insert_pdf(src_doc, from_page=src_idx, to_page=src_idx)

    extra_pages_added = 0
    insert_offset = 0

    for out_idx, src_idx in enumerate(highlighted_indices):
        ...
        dst_page.insert_textbox(
            ...
            f"Page {src_idx + 1}",
            fontsize=LABEL_FONT_SIZE, fontname="helv",
            color=(0.4, 0.4, 0.4), align=fitz.TEXT_ALIGN_RIGHT,
        )
    ...
    print(f"  {total_pages} total pages, {len(highlighted_indices)} with highlights, {extra_pages_added} continuation pages")
    print(f"  Included pages: {', '.join(str(i + 1) for i in highlighted_indices)}")
